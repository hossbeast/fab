...
{
  if(a->s)
    xunit_info_pushs(name, a->s);
  else
    xunit_info_pushs(name, "(null)");
}
...
static void stringl_info_push(const char * const restrict name, xunit_arg * a)
{
  ...
  {
    xunit_info_pushf(name, "%.*s", (int)a->l, (char*)a->p);
  }
}
...
static void char_info_push(const char * const restrict name, xunit_arg * a)
{
  ...
  {
    xunit_info_pushf(name, "0x%02hhx", a->c);
  }
  else
  {
    xunit_info_pushf(name, "'%c'", a->c);
  }
}
...
{
  xunit_info_pushf(name, "%"PRIi32, a->i32);
}
...
{
  xunit_info_pushf(name, "%"PRIu32, a->u32);
}
...
{
  xunit_info_pushf(name, "%"PRIi64, a->i64);
}
...
{
  xunit_info_pushf(name, "%"PRIu64, a->u64);
}
...
{
  xunit_info_pushs(name, a->b ? "true" : "false");
}
