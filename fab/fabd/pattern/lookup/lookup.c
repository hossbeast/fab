...
//char path[512];
//fsent_absolute_path_znload(path, sizeof(path), containerof(v, fsent, vertex));
//printf("consider %s ", path);
...
)
{
  ...
  *errlen = snprintf(err, errsz, " location %s line %d\n", fname, loc->f_lin + 1);
  ...
  {
    sz = fsent_path_znload(path1, sizeof(path1), vertex_value(vertices[0]));
    xapi_info_pushw("one", path1, sz);

    sz = fsent_path_znload(path2, sizeof(path2), vertex_value(vertices[1]));
