...
xapi rule_module_edge_alloc(rule_module_edge ** restrict rmep)
{
  ...
  llist_init_node(&rme->lln_refresh);
...
//    fatal(dependency_disconnecting, dep, invalidation);
  ...
  fatal(graph_disconnect, &rme->edge); // rma->associating_edge);
  rule_module_edge_release(rme);
