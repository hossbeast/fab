...
xapi formula_create(formula ** restrict fmlp, moria_graph * restrict g)
{
  ...
  snprintf(name, sizeof(name), "(formula %p)", fml);

  moria_vertex_inits(&fml->vertex, g, VERTEX_FML, name);
...
xapi formula_xrelease(formula * restrict fml)
{
  enter;
