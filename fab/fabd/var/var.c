...
int var_access(const map * const restrict vmap, const char * restrict vs, lwx *** const restrict stax, size_t * const restrict staxa, size_t * const restrict staxp, lwx ** const restrict ls)
{
  const map * m = vmap;
  const char * s = vs;
