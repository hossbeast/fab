...
static xapi __attribute__((nonnull)) bpe_setup(buildplan_entity * restrict bpe, node * restrict fmlnode)
{
  enter;
      ...
      else if(rule->card == RULE_ONE_TO_MANY)
      {
        ...
        if(log_would(L_RULE))
        {
          for(y = 0; y < z; y++)
          {
            fatal(log_start, L_RULE, &N);
            xsays("   ");
            fatal(node_path_say, Alist[y], N);
            xsayf(" var %s", Alist[y]->var ? Alist[y]->var->norm : "(none)");
            fatal(log_finish);
          }
        ...
        if(rule->fml_node)
        {
          ...
          {
            if(!Alist[y]->bpe)
              fatal(bpe_attach, Alist[y], &ne->bpe, &ctx->invalidation);
          }
        }
      }
      else
