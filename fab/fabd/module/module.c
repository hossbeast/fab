...
//if(block) {
//  printf("block variants %zu\n", block->variants->table_size);
//}
...
//printf("USES dir-node %p mod %p dir-node attrs 0x%08x\n", mod_dir_n, mod_dir_n->mod, mod_dir_v->attrs);
...
//char path[512];
//fsent_project_relative_path_znload(path, sizeof(path), vertex_value(mod_dir_v));
//printf("[%d] %s\n", fsent_invalid_get(vertex_value(mod_dir_v)), path);
...
static xapi module_rules_dismantle(module * restrict mod, module_parser * restrict parser, graph_invalidation_context * restrict invalidation)
{
  ...
  if(fsent_fsenttype_get(mod->self_node) == VERTEX_FSENTTYPE_MODULE) {
    /*
     * visit rules connected to this module, including rules defined by the module, and rules
     * defined by models used by the module
     */
    llist_init_node(&list);
    fatal(moria_traverse_vertex_edges
      , &g_graph
    ...
    /* disconnect rmas connected (but not owned by) this module */
  ...
  {
    RUNTIME_ASSERT(fsent_fsenttype_get(mod->self_node) == VERTEX_FSENTTYPE_MODEL);
...
/* release blocks in the module */
static void module_guts_dismantle(module * restrict mod, module_parser * restrict parser)
{
...
static xapi module_reparse(module * restrict mod, module_parser * restrict parser, graph_invalidation_context * restrict invalidation, bool * restrict reconciled)
{
  ...
  /* release the guts */
  module_guts_dismantle(mod, parser);
...
//  /* traversing the module vertex seamlessly transfers to the module directory fsent */
//  vertex_containerof(mod)->ref = vertex_containerof(mod->dir_node);
...
xapi module_system_reconcile(graph_invalidation_context * restrict invalidation, bool * restrict reconciled)
{
  ...
  llist_foreach(&module_list, mod, vertex.owner) {
    ...
    if(mod->self_node->not_parsed) {
      *reconciled = false;
      fprintf(stderr, "module(s) not parsed\n");
      goto XAPI_FINALIZE;
    }
...
//  len = fsent_absolute_path_znload(dst + z, sz - z, mod->dir_node);
...
//  /* abspath */
//  z += marshal_u16(dst + z, sz - z, fml->abspath_len);
//  z += znloadw(dst + z, sz - z, fml->abspath, fml->abspath_len);
...
xapi module_resolve_require(module * restrict A, module * restrict B, graph_invalidation_context * restrict invalidation)
{
  ...
  r = moria_preconnect(&ctx, &g_graph, &A->vertex, &B->vertex, EDGE_REQUIRES, 0);
  RUNTIME_ASSERT(r == MORIA_NOEDGE);
...
//    // attach to (or override in) //module/use-scope/
//    fatal(shadow_graft_use_scope, A, B->dir_node, refname, refname_len, true, &se, invalidation);
  }

  r = moria_preconnect(&ctx, &g_graph, &A->vertex, &B->vertex, EDGE_USES, 0);
  RUNTIME_ASSERT(r == MORIA_NOEDGE);
  fatal(module_edge_alloc, &nem);
...
//    // attach to (or override in) //module/import-scope/
//    fatal(shadow_graft_import_scope, A, B, refname, refname_len, true, &se, invalidation);
  }

  r = moria_preconnect(&ctx, &g_graph, &A->vertex, &B->vertex, EDGE_IMPORTS, 0);
  RUNTIME_ASSERT(r == MORIA_NOEDGE);
  fatal(module_edge_alloc, &nem);
...
//  vertex_containerof(mod)->ref = vertex_containerof(mod->dir_node);
  ...
  {
    fsent_kind_set(mod_file_n, VERTEX_MODULE_FILE);
//printf("BOOTSTRAP module dir-node %p mod %p %.16s %s\n", mod_dir_n, mod_dir_n->mod, mod->id, mod->dir_node_abspath);
  }
  else if(memncmp(mod_file_n->name.name, mod_file_n->name.namel, fsent_model_name, fsent_model_name_len) == 0)
  {
    fsent_kind_set(mod_file_n, VERTEX_MODEL_FILE);
//printf("BOOTSTRAP model dir-node %p mod %p %.16s %s\n", mod_dir_n, mod_dir_n->mod, mod->id, mod->dir_node_abspath);
  }
  else
  {
    RUNTIME_ABORT();
...
//printf("initializing %p %s\n", mod, mod->self_node_relpath);
  // build out the modules shadow node
  fatal(shadow_module_init, mod, invalidation);

  // build out the shadow scope/ dirs from top-level names
  scope_ctx.mod = mod;
...
#if 0
forget about keeping these live unless an integ test needs it

  /* mirror into module shadow scopes */
  if(mod && fsent_parent(n) == mod->dir_node)
  {
//printf("LINKING mirror %s\n", n->name.name);
    fatal(shadow_graft_import_scope, mod, n, n->name.name, n->name.namel, false, &n->import_scope_edge, invalidation);
    fatal(shadow_graft_use_scope, mod, n, n->name.name, n->name.namel, false, &n->use_scope_edge, invalidation);
  }
#endif

#if 0
/*
 * attach nodes in the modules root directory to shadow scope nodes (no collisions are possible yet)
 */
static xapi module_root_graft(moria_vertex * v, void * ctxp, moria_traversal_mode mode, int distance, int * result)
{
  enter;
  ...
  n = containerof(v, typeof(*n), vertex);
printf("MODULE mirror %.*s\n", v->label_len, v->label);
  fatal(shadow_graft_import_scope, ctx->mod, n, n->name.name, n->name.namel, false, &n->import_scope_edge, ctx->invalidation);
  fatal(shadow_graft_use_scope, ctx->mod, n, n->name.name, n->name.namel, false, &n->use_scope_edge, ctx->invalidation);
