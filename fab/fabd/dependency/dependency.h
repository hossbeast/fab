/* Copyright (c) 2012-2015 Todd Freed <todd.freed@gmail.com>

   This file is part of fab.

   fab is free software: you can redistribute it and/or modify
   it under the terms of the GNU General Public License as published by
   the Free Software Foundation, either version 3 of the License, or
   (at your option) any later version.

   fab is distributed in the hope that it will be useful,
   but WITHOUT ANY WARRANTY; without even the implied warranty of
   MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
   GNU General Public License for more details.

   You should have received a copy of the GNU General Public License
   along with fab.  If not, see <http://www.gnu.org/licenses/>. */

#ifndef FABD_BUILDPLAN_ENTITY_H
#define FABD_BUILDPLAN_ENTITY_H

#include "xapi.h"
#include "types.h"
#include "valyria/llist.h"
#include "moria.h"

#include "graph.h"

struct narrator;
struct fsent;
struct llist;
struct rule_module_edge;
struct graph_invalidation_context;

// active list
extern llist dependency_list;

/*
 * EDGE_TYPE_DEPENDS - the targets and sources are the upwards and downwards nodes on the edge
 * respectively. If a formula is attached, the dependency can be added to the buildplan
 */
typedef struct dependency {
  moria_edge edge;

  const struct rule_module_edge * rme;      // not null - rme which created this edge
  llist dependencies_lln;                   // in rme->dependencies
  uint32_t reconciliation_id;

  /* dependency with an attached fml can be added to the buildplan */
  const struct fsent *fml;
  uint16_t id;              // build plan id
  uint16_t depth;           // max depth
} dependency;

xapi dependency_alloc(dependency ** restrict rv, struct moria_graph * restrict g, uint16_t Alen, uint16_t Blen)
  __attribute__((nonnull));

void dependency_release(dependency * restrict dep)
  __attribute__((nonnull));

void dependency_sources(const dependency * restrict bpe, struct llist * restrict list)
  __attribute__((nonnull));

void dependency_targets(const dependency * restrict bpe, struct llist * restrict list)
  __attribute__((nonnull));

xapi dependency_say_sources(const dependency * restrict bpe, struct narrator * restrict N)
  __attribute__((nonnull));

xapi dependency_say_targets(const dependency * restrict bpe, struct narrator * restrict N)
  __attribute__((nonnull));

xapi dependency_connect(
    struct fsent * restrict above
  , struct fsent * restrict below
  , enum edge_kind relation
  , struct graph_invalidation_context * restrict invalidation
  , struct dependency ** restrict e
)
  __attribute__((nonnull));

xapi dependency_hyperconnect(
    struct moria_vertex ** restrict Alist
  , uint16_t Alen
  , struct moria_vertex ** restrict B
  , uint16_t Blen
  , enum edge_kind relation
  , struct graph_invalidation_context * restrict invalidation
  , struct dependency ** restrict e
)
  __attribute__((nonnull(6)));

xapi dependency_cleanup(void);

xapi dependency_disconnect(
    dependency * restrict ne
  , struct graph_invalidation_context * restrict invalidation
)
  __attribute__((nonnull));

#endif
