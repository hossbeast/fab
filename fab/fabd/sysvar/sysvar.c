...
xapi exec_render_sysvar_sources(exec_render_context * restrict ctx, const build_slot * restrict bs)
{
  ...
  if(!(e->attrs & MORIA_EDGE_HYPER))
  {
    ctx->builder_add_args.val.n = containerof(e->B, fsent, vertex);
  }
  else if(e->Blen == 1)
  {
    ctx->builder_add_args.val.n = containerof(e->Blist[0].v, fsent, vertex);
  }

  ctx->builder_add_args.render_val = RENDER_PROPERTY;
  ctx->builder_add_args.val.prop = FSENT_PROPERTY_RELPATH;
  ...
  else
  {
    for(x = 0; x < e->Blen; x++)
    {
      ctx->builder_add_args.val.n = containerof(e->Blist[x].v, fsent, vertex);
      fatal(builder_add, ctx->builder, &ctx->builder_add_args);
    }
  }
...
xapi exec_render_sysvar_targets(exec_render_context * restrict ctx, const build_slot * restrict bs)
{
  ...
  if(!(e->attrs & MORIA_EDGE_HYPER))
  {
    ctx->builder_add_args.val.n = containerof(e->A, fsent, vertex);
  }
  else if(e->Alen == 1)
  {
    ctx->builder_add_args.val.n = containerof(e->Alist[0].v, fsent, vertex);
  }
  ...
  else
  {
    for(x = 0; x < e->Alen; x++)
    {
      ctx->builder_add_args.val.n = containerof(e->Alist[x].v, fsent, vertex);
      fatal(builder_add, ctx->builder, &ctx->builder_add_args);
    }
  }
...
xapi exec_render_sysvar_variant(exec_render_context * restrict ctx, const build_slot * restrict bs)
{
  ...
  {
    ctx->builder_add_args.render_val = RENDER_PROPERTY;
    ctx->builder_add_args.val.prop = FSENT_PROPERTY_VARIANT;
    ctx->builder_add_args.mode = BUILDER_APPEND;
