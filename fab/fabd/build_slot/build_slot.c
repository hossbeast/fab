...
xapi build_slot_prep(build_slot * restrict bs, dependency * restrict dep, uint32_t stage_index)
{
  ...
  else
  {
    if(e->Alen) {
      ...
      n = containerof(e->Blist[0].v, fsent, vertex);
    }
    bs->var = n->var;

    /* nodes in a hyperedge need not all belong to the same variant ; only set the variant on the
     * buildslot if all nodes agree */
    for(x = 0; x < e->Alen; x++)
    {
  ...
  if((bs->mod = fsent_module_get(n)) == 0)
  {
