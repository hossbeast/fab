...
//printf("[%d] BOOTSTRAP THREAD\n", tid);
  /* bootstrap filesystem trees containing config files */
  fatal(config_system_bootstrap);

  // queue up project module for reconcile
  //fatal(module_system_bootstrap, g_project_root, g_params.proj_dir);

//  // load filesystem trees
//  fatal(node_system_reload);

  /* re-load everything */
//printf("[%d] BOOTSTRAP system-reload\n", tid);
  fatal(global_system_reconcile, ctx, &reconciled);

//  fatal(config_system_bootstrap_late);
//printf("[%d] BOOTSTRAP done\n", tid);
