  ...
  struct stat stb;
	...
	int fn(const char * fpath, const struct stat * sb, int typeflat, struct FTW * ftwbuf)
	{
		if(ftwbuf->level == 1)
		{
			...
			typeof(ctx->cmds[0]) * T = &ctx->cmds[ctx->cmdsl];
			T->num = num;
			T->cmd_fd = -1;
			T->stdo_fd = -1;
			T->stde_fd = -1;
  ...
  fatal(executor_context_recycle, ctx);
	...
	for(j = 0; j < ctx->cmdsl; j += step)
	{
		...
		for(i = j; i < (j + cmdsl); i++)
		{
			...
			fatal(psloadf, &ctx->tmp, XQUOTE(FABTMPDIR) "/pid/%d/bp/%d/%d/out", g_params.pid, stagex, i);
			...
			fatal(psloadf, &ctx->tmp, XQUOTE(FABTMPDIR) "/pid/%d/bp/%d/%d/err", g_params.pid, stagex, i);
			...
			// path to the command
			fatal(psloadf, &ctx->tmp, XQUOTE(FABTMPDIR) "/pid/%d/bp/%d/%d/cmd", g_params.pid, stagex, i);
		...
		while((pid = wait(&r)) != -1)
		{
			for(i = j; i < (j + step) && i < ctx->cmdsl; i++)
			{
			...
			fatal(psloadf, &ctx->tmp, XQUOTE(FABTMPDIR) "/pid/%d/bp/%d/%d/exit", g_params.pid, stagex, i);
			fatal(ixclose, &fd);
        ...
        int y;
				for(y = 0; y < ctx->cmds[i].prodl; y++)
				{
					if(y == 0)
					{
						...
						if(bad)
						{
							logf(0, ", details @ %s/bp/%d/%d", g_params.ipcstem, stagex, i);
						}
