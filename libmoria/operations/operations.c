...
//printf("%s:%d\n", __FUNCTION__, __LINE__);
    ...
    {
//printf("%s:%d e->B %p B %p\n", __FUNCTION__, __LINE__, e->B, B);
    }
  }
...
)
{
  ...
  {
    RUNTIME_ASSERT(rbtree_put_node(&B->up, &e->rbn_up, edge_cmp_rbn_up) == 0);
  }
...
//printf("%s:%d Alen %d Blen %d\n", __FUNCTION__, __LINE__, Alen, Blen);
