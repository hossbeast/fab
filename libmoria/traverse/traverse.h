 ...
 */
xapi moria_vertex_count(struct moria_vertex * restrict v, void * ctx, moria_traversal_mode mode, int distance, int * restrict);
xapi moria_vertex_count_once(struct moria_vertex * restrict v, void * ctx, moria_traversal_mode mode, int distance, int * restrict);

/*
 * Callback to count edges visited - ctx points to a uint32_t
 */
xapi moria_edge_count(struct moria_edge * restrict v, void * ctx, moria_traversal_mode mode, int distance, int * restrict);
xapi moria_edge_count_once(struct moria_edge * restrict v, void * ctx, moria_traversal_mode mode, int distance, int * restrict);

//xapi graph_vertex_traversal_state_xfree(moria_vertex_traversal_state * restrict state);
//xapi graph_vertex_traversal_state_ixfree(moria_vertex_traversal_state ** restrict state)
//  __attribute__((nonnull));
//
//xapi graph_edge_traversal_state_xfree(moria_edge_traversal_state * restrict state);
//xapi graph_edge_traversal_state_ixfree(moria_edge_traversal_state ** restrict state)
//  __attribute__((nonnull));
...
bool moria_traversal_vertex_visited(const struct moria_graph * const restrict g, const struct moria_vertex * const restrict v, moria_vertex_traversal_state * restrict state)
  __attribute__((nonnull));

bool moria_traversal_edge_visited(const struct moria_graph * const restrict g, const struct moria_edge * const restrict v, moria_edge_traversal_state * restrict state)
  __attribute__((nonnull));
...
//
xapi moria_traverse_vertices_all(
    /* 1 */ struct moria_graph * restrict g
  , /* 2 */ xapi (* visitor)(struct moria_vertex * restrict, void * ctx, moria_traversal_mode mode, int distance, int * restrict)
  , /* 3 */ const moria_traversal_criteria * restrict criteria
  , /* 4 */ uint32_t attrs
  , /* 5 */ void * ctx
)
