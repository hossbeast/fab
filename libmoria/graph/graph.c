...
void API moria_graph_init(moria_graph * const restrict g)//, uint32_t identity)
{
  memset(g, 0, sizeof(*g));
//  g->identity = identity;
...
xapi API moria_graph_create(moria_graph ** const restrict g)//, uint32_t identity)
{
  enter;
...
//printf("%s:%d %.*s not-found in mm %p\n", __FUNCTION__, __LINE__, (int)label_len, label, mm);
    goto XAPI_FINALIZE;
  }
//printf("%s:%d %.*s found in mm %p\n", __FUNCTION__, __LINE__, (int)label_len, label, mm);

  rbtree_foreach(&entry->rbt, v, rbn_lookup) {
    if(candidate_callback && !candidate_callback(context, v)) {
//printf(" 1 rejected candidate %.*s\n", (int)v->label_len, v->label);
      continue;
    }
...
//printf(" %s:%d %.*s <=> %.*s\n", __FUNCTION__, __LINE__, (int)label_len, label, (int)p->label_len, p->label);
        break;
      }
    }
...
xapi API moria_graph_linear_search(llist * restrict vertex_list, const char * restrict label, uint16_t label_len, moria_vertex ** restrict rv)
{
  ...
  *rv = 0;
//printf("list len %zu\n", llist_count(vertex_list));
  llist_foreach(vertex_list, v, owner) {
//printf(" > %.*s <=> %.*s\n", (int)label_len, label, (int)v->label_len, v->label);
