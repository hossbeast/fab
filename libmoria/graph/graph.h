 ...
 */
xapi moria_graph_create(moria_graph ** const restrict g)//, uint32_t identity)
  __attribute__((nonnull));

void moria_graph_init(moria_graph * const restrict g)//, uint32_t identity)
  __attribute__((nonnull));

/*
 * [defs]      - name/value mappings for parse and say
 */
void moria_graph_vertex_definitions_set(moria_graph * restrict g, const struct attrs32 * restrict defs)
  __attribute__((nonnull));

/*
 * [defs]      - name/value mappings for parse and say
 */
void moria_graph_edge_definitions_set(moria_graph * restrict g, const struct attrs32 * restrict defs)
  __attribute__((nonnull));

void moria_graph_destroy(moria_graph * const restrict g)
  __attribute__((nonnull));
...
//  free a graph
//
xapi moria_graph_xfree(moria_graph * const restrict g);
...
//
xapi moria_graph_ixfree(moria_graph ** const restrict g)
  __attribute__((nonnull));
...
xapi moria_graph_say(
  /* 1 */   moria_graph * const restrict g
  /* 2 */ , llist ** restrict vertex_lists
  /* 3 */ , uint16_t vertex_lists_len
  /* 4 */ , llist ** restrict edge_lists
...
#if 0
uint32_t moria_vertex_key_hash(uint32_t h, const void *key, size_t sz)
  __attribute__((nonnull));
#endif
...
xapi moria_graph_linear_search(llist * restrict vertex_list, const char * restrict label, uint16_t label_len, struct moria_vertex ** restrict rv)
  __attribute__((nonnull));
