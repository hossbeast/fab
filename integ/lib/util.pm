...
#print("rd($err_rd_fd) : $er\n");
  ...
  if(   (!$$params{expect_error} && !$$params{tolerate_error} && $status_by_pid{$pid})
     || ($$params{expect_error} && !$status_by_pid{$pid}))
  {
    if($$params{cwd}) {
      print STDERR ("cwd : $$params{cwd}\n");
    }
